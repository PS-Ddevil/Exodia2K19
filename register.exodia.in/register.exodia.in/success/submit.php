...
if($auth->isLoggedIn()) {
    $conn = new mysqli("localhost", "register", "root@exodia12345", "event_registeration");
	...
	setcookie("college", $college, time() + (86400 * 30), "/");
	setcookie("manager", $manager, time() + (86400 * 30), "/");
    $search = "select * from event_registeration where email = '$email' and event = '$event' and payment='yes'";
        $result = $conn->query($search);
        if (!($result->num_rows>0)) {
			$_SESSION["event"] = $event;
            $_SESSION["txn_amount"] = $_POST["amount"];
            if((int)$_POST["amount"]>0 or $event=="Accomodation") {
				$sql = "insert into event_registeration (email, name, phone, college, manager, event, message) values ('$email', '$name', '$phone', '$college', '$manager', '$event', '$message')";
            $conn->query($sql);
                    header("Location: https://register.exodia.in/login/pay");
			}
			else {
            $sql = "insert into event_registeration (email, name, phone, college, manager, event, message, payment) values ('$email', '$name', '$phone', '$college', '$manager', '$event', '$message', 'Yes')";
            $conn->query($sql);
            $info = "Successfully Registered!<br>For Accomodation go to Accomodation link at <a href='https://register.exodia.in/login/home/'><ul>Home Page</ul></a>";
...
try {
    ...
    $mail->Body    = 'Greetings! <br><br>Thank you for registering for ('. $event.'). Book Accomodation <a href="https://register.exodia.in/login/home/">Here</a><br><br>Team Exodia';
    $mail->send();
} catch (Exception $e) {
}}
        }
        else {
            $info = "Entry already exists in database";
        }
}
else {
...
<!DOCTYPE html>
<head>
    <title>Response Exodia'19</title>
...
<body class="permission_denied">
    ...
    <div class="denied__wrapper">
      <h3><?php echo $info;?></h3>
      <br>
      <!-- <h3><ul><li>You will be informed regarding payment within few days.</li></ul></h3> -->
      <!-- <h5 style="color:white"><ul><li>Incorrect Login</li><li>Server Error</li><li>Trying to excess link that's not available</li></ul></h5> -->
      ...
      <a href="https://register.exodia.in/login/home"><button class="denied__link">Register for another event</button></a>
    </div>
</body>
</html>